...
class WeatherUI(QWidget):
    ...
    def update_weather_ui(self, data):
        ...
        self.time_label.setText(f"Pogoda teraz - dzisiaj godz. {data['current_time']}")
        self.sunrise_label.setText(f"{data['sunrise']}")
        self.sunset_label.setText(f"{data['sunset']}")
        self.weather_condition.setText(data["description"].capitalize())
        self.temperature_label.setText(f"ODCZUWALNA {data['feels_like']}°C")
        self.humidity_value.setText(f"{data['humidity']}%")
        self.pressure_value.setText(f"{data['pressure']} hPa")
        self.wind_value.setText(f"{data['wind_speed']} km/h")